  ...
  return (
    <nav aria-label="Main" className="px-4 py-4 md:px-6 md:py-6">
      <div className="mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-white md:flex-row md:items-center">
        ...
        <div
          className={clsx(
            "ga-4 pt-15 fixed bottom-0 left-0 right-0 top-0 z-40 flex flex-col items-end bg-[#070815] pr-4 transition-transform duration-300 ease-in-out motion-reduce:transition-none md:hidden",
          ...
          <div className="grid justify-items-end gap-8">
            {settings.data.navigation.map((item) => {
              ...
              return (
                <PrismicNextLink
                  key={item.label}
                  className="block px-3 text-3xl first:mt-12"
                  field={item.link}
                  onClick={() => setOpen(false)}
                  aria-current={
                    pathname.includes(asLink(item.link) as string)
